 ...
 * 
 * Run with: node scripts/normalize-times.mjs
 */
...
async function main() {
  ...
  const [allOrders] = await connection.execute(
    'SELECT id, orderNumber, appointmentTime FROM orders WHERE appointmentTime IS NOT NULL AND appointmentTime != ""'
  );
  ...
  console.log(`   Total orders with appointment times: ${allOrders.length}`);
  ...
  for (const order of ordersToUpdate) {
    try {
      await connection.execute(
        'UPDATE orders SET appointmentTime = ? WHERE id = ?',
